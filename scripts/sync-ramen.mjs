// sync-ramen.mjs
// 全国のラーメン・つけ麺・担々麺などの麺料理店をOpenStreetMapから取得してSupabaseに保存
...
async function fetchRestaurants(bbox, region) {
  const query = `
    [out:json][timeout:90];
    (
      node["amenity"="restaurant"]["cuisine"="ramen"](${bbox});
      node["amenity"="restaurant"]["cuisine"="noodle"](${bbox});
      node["amenity"="restaurant"]["cuisine"="chinese"](${bbox});
      node["amenity"="restaurant"]["cuisine"="japanese"](${bbox});
      node["amenity"="fast_food"]["cuisine"="ramen"](${bbox});
      node["amenity"="fast_food"]["cuisine"="noodle"](${bbox});
      node["amenity"="restaurant"]["name"~"ラーメン|らーめん|拉麺|麺|つけ麺|担々麺|担担麺|ちゃんぽん",i](${bbox});
    );
    out body;
  `;
  ...
  for (const region of REGION_BBOXES) {
    ...
    try {
      ...
      for (let i = 0; i < restaurants.length; i += 1000) {
        ...
        const { error } = await supabase.from('ramen_restaurants').upsert(chunk);
